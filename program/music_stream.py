        ...
        if chat_id in QUEUE:
            ...
            try:
                ...
                await m.reply_photo(
                    photo=image,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=f"🗂 **Title::** [{songname}]({link}) | `music`\n"
                            f"⏱️ **Duration:** `{duration}`\n"
                            f"🧸 **Request by:** {requester}",
                )
                remove_if_exists(image)
...
@Client.on_message(command(["play", "p", f"play@{BOT_USERNAME}", f"p@{BOT_USERNAME}"]) & other_filters)
@check_blacklist()
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                ...
                                await m.reply_photo(
                                    photo=image,
                                    reply_markup=InlineKeyboardMarkup(buttons),
                                    caption=f"🗂 **Title:** [{songname}]({url}) | `music`\n**⏱ Duration:** `{duration}`\n🧸 **Request by:** {requester}",
                                )
                                remove_if_exists(image)
                            except (NoActiveGroupCall, GroupCallNotFound):
                                await suhu.delete()
            ...
            if "m3u8" in url:
                if chat_id in QUEUE:
                    ...
                    await m.reply_photo(
                        photo=f"{IMG_1}",
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=f"💡 **Track added to queue »** `{pos}`\n\n🗂 **Title:** [m3u8 audio stream]({url}) | `live`\n🧸 **Requested by:** {requester}",
                    )
                else:
                    try:
                        ...
                        await m.reply_photo(
                            photo=f"{IMG_2}",
                            reply_markup=InlineKeyboardMarkup(buttons),
                            caption=f"🗂 **Title:** [m3u8 audio stream]({url}) | `live`\n🧸 **Requested by:** {requester}",
                        )
                    except (NoActiveGroupCall, GroupCallNotFound):
                        await msg.delete()
                        await remove_active_chat(chat_id)
